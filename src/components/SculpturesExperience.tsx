...
export function SculpturesExperience() {
  // 'TPP' = Third Person Perspective (Overview of Gallery with Observer)
  // 'FPP' = First Person Perspective (Zoomed in past Observer with 3D Outward Curved Liquid Glass Carousel)
  const [viewMode, setViewMode] = useState<'TPP' | 'FPP'>('TPP');
  const [isTransitioning, setIsTransitioning] = useState<boolean>(false);
  const [rotation, setRotation] = useState<number>(0);

  // Physics Refs for continuous spinner feel
  ...
  // High-performance continuous animation loop (60/120fps physics)
  useEffect(() => {
  ...
  // Handle Switch View between TPP and FPP
  ...
  // Keyboard navigation when in FPP
  useEffect(() => {
  ...
  // Fast, free, continuous momentum wheel spinner physics
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      // Fast continuous momentum spin (spins rapidly with wheel velocity)
      targetRotationRef.current += delta * 0.004;

      // Magnetic snap to nearest sculpture card when spinning slows down
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 relative z-10">
        ...
        <Reveal direction="zoom" delay={0.2}>
          <div className="relative w-full overflow-hidden rounded-2xl sm:rounded-3xl lg:rounded-[2.5rem] border border-white/15 bg-[#030405] shadow-[0_30px_90px_rgba(0,0,0,0.95)]">
            ...
            {/* Viewport Box (with fixed aspect ratio and min-height) */}
            <div className="relative w-full aspect-[4/3] sm:aspect-[16/10] md:aspect-[21/11] min-h-[540px] sm:min-h-[620px] md:min-h-[700px] overflow-hidden select-none">
              ...
              {/* ========================================================================= */}
              {/* 1. CAMERA / BACKGROUND LAYER (PARALLAX ZOOM ONTO CENTER SCULPTURE)        */}
              {/* ========================================================================= */}
              <div
                className="absolute inset-0 overflow-hidden transition-all ease-[cubic-bezier(0.22,1,0.36,1)] will-change-transform"
                style={{
                  transformOrigin: '49.6% 47.5%', // Fixed precisely on the grand center sculpture on the pedestal
                  transform:
                    viewMode === 'FPP'
              ...
              >
                ...
                {/* Subtle Ambiance Gradient */}
                <div
                  className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-black/25 pointer-events-none"
                />
              </div>

              {/* Parallax Zoom Transition Glow */}
              <div
              ...
              {/* ========================================================================= */}
              {/* 2. TPP MODE OVERLAY: OBSERVER FOCUS & "SHIFT MY VIEW" BUTTON             */}
              {/* ========================================================================= */}
              {viewMode === 'TPP' && (
                <div className="absolute inset-0 flex flex-col justify-between p-5 sm:p-8 md:p-10 pointer-events-none transition-all duration-500 opacity-100">
                  
                  {/* Top Status Pill */}
                  <div className="flex items-center justify-between w-full">
                    <div className="liquid-glass-pill px-3.5 sm:px-4 py-1.5 rounded-full inline-flex items-center gap-2 text-[10px] sm:text-xs tracking-[0.25em] uppercase text-[#f1eee7]/90">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#b89a62] animate-pulse" />
                      Third-Person View (TPP)
                    </div>
                    <div className="hidden sm:inline-flex liquid-glass-pill px-3.5 py-1.5 rounded-full text-[10px] sm:text-xs tracking-[0.2em] uppercase text-[#b9b5ae]">
                      Target: Center Marble Deity
                    </div>
                  </div>

                  {/* Center Observer Pulse Indicator (Targeted directly to the observer in TPP) */}
                  <div className="absolute top-[71%] left-[41.8%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center pointer-events-none">
                    <div className="relative flex items-center justify-center">
                      <span className="absolute w-12 h-12 rounded-full border border-[#b89a62]/60 animate-ping" />
                      <span className="w-4 h-4 rounded-full bg-[#b89a62]/50 border border-[#b89a62] backdrop-blur-sm shadow-[0_0_15px_rgba(184,154,98,0.8)]" />
                    </div>
                    <span className="mt-2 text-[9px] tracking-[0.25em] uppercase text-[#b89a62] bg-black/70 px-2 py-0.5 rounded border border-[#b89a62]/30 backdrop-blur-md whitespace-nowrap">
                      Observer (TPP)
                    </span>
                  </div>
              ...
              {/* ========================================================================= */}
              {/* 3. FPP MODE OVERLAY: 3D OUTWARD CURVED LIQUID GLASS CAROUSEL             */}
              {/* ========================================================================= */}
              {viewMode === 'FPP' && (
                <div
                ...
                >
                  ...
                  <div className="flex items-center justify-between w-full relative z-30">
                    ...
                    {/* Active Sculpture Index Indicator */}
                    ...
                    {/* Exit to TPP / Return Overview Button */}
                    <button
                      onClick={handleToggleView}
                      disabled={isTransitioning}
                  ...
                  {/* 3D OUTWARD CURVED LIQUID GLASS CAROUSEL (PORTRAIT ORIENTATION + INFINITE SCROLL) */}
                  <div
                    ref={stageRef}
                  ...
                  >
                    <div className="relative w-full h-full max-h-[440px] sm:max-h-[500px] md:max-h-[550px] flex items-center justify-center [transform-style:preserve-3d]">
                      {SCULPTURE_CAROUSEL_ITEMS.map((item, index) => {
                        ...
                        const isVisible = Math.abs(diff) <= 2.6; // 5 cards continuous visible stream

                        // Continuous fluid 3D geometry for high-speed spinner
                        ...
                        return (
                          ...
                          >
                            {/* PURE CLEAN TRANSPARENT WHITE LIQUID GLASS PANEL */}
                            <div className="relative w-full h-full rounded-2xl overflow-hidden backdrop-blur-2xl bg-[linear-gradient(135deg,rgba(255,255,255,0.08)_0%,rgba(255,255,255,0.02)_50%,rgba(255,255,255,0.05)_100%)] p-4 sm:p-5 flex flex-col items-center justify-center">
                              ...
                              {/* Inner Glass Viewport Frame (Clean & Empty) */}
